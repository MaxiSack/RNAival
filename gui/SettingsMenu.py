...
class SettingsMenu():
	def __init__(self,main):
		...
		#showGraphsOnProjectLoad: radio
		ThemedLabel(mainframe,text="Show graphs on project load:",anchor="w").grid(column=0,row=2,columnspan=1,sticky="ew",padx=main.frameBorderSize)
		showGraphsVar = self.PM.add("showGraphsOnProjectLoad","bool",main.PM.get("showGraphsOnProjectLoad"),
					"Bool error","Wether to generate the graphs on project load or not.",tag="general")
		ThemedRadioButton(mainframe,text="Yes",variable=showGraphsVar,value=True).grid(column=1,row=2,sticky="w",padx=main.frameBorderSize)
		ThemedRadioButton(mainframe,text="No",variable=showGraphsVar,value=False).grid(column=2,row=2,sticky="w",padx=main.frameBorderSize)
		...
		ThemedLabel(mainframe,text="Threads to use:",anchor="w").grid(column=0,row=3,columnspan=1,sticky="w",padx=main.frameBorderSize)
		...
		mainframe.rowconfigure(4,weight=0,uniform="fred")
